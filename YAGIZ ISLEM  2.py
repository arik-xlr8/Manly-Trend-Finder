...
class PivotEngine:
    """
    PivotEngine:
    - MAX ve MIN pivot adaylarını (candidate) takip eder
    - 2 mum sonrası pivot onayı yapar (confirm)
    - Aday pivot ilerlerse güncellenir (A seçeneği)
    - Onaylanmış pivot kırılırsa (price break) pivot silinir
    - Aynı anda hem MAX hem MIN candidate tutulabilir
    """
    ...
    def __init__(self, confirm_bars: int = 2):
        ...
        # Candidate MAX
        self.cand_max_idx: Dict[str, Optional[Any]] = {}
        self.cand_max_price: Dict[str, float] = {}
        self.cand_max_age: Dict[str, int] = {}

        # Candidate MIN
        self.cand_min_idx: Dict[str, Optional[Any]] = {}
        self.cand_min_price: Dict[str, float] = {}
        self.cand_min_age: Dict[str, int] = {}

        # Confirmed MAX
        self.conf_max_idx: Dict[str, Optional[Any]] = {}
        self.conf_max_price: Dict[str, float] = {}

        # Confirmed MIN
        self.conf_min_idx: Dict[str, Optional[Any]] = {}
        self.conf_min_price: Dict[str, float] = {}

    # ----------------------------------------------------
    def _init_sym(self, sym: str):
        self.cand_max_idx.setdefault(sym, None)
        self.cand_max_price.setdefault(sym, 0.0)
        self.cand_max_age.setdefault(sym, 0)

        self.cand_min_idx.setdefault(sym, None)
        self.cand_min_price.setdefault(sym, 0.0)
        self.cand_min_age.setdefault(sym, 0)

        self.conf_max_idx.setdefault(sym, None)
        self.conf_max_price.setdefault(sym, 0.0)

        self.conf_min_idx.setdefault(sym, None)
        self.conf_min_price.setdefault(sym, 0.0)

    # ----------------------------------------------------
    def safe_get_pos(self, df: pd.DataFrame, idx_label):
        try:
            _ = df.iloc[int(idx_label)]
            return int(idx_label)
        except Exception:
            try:
    ...
    # ====================================================
    def update(self, df: pd.DataFrame, sym: str, filtered_signal: Optional[Dict] = None):
        self._init_sym(sym)
        events = {
            'new_confirmed_max': None,
            'new_confirmed_min': None,
            'deleted_confirmed_max': None,
            'deleted_confirmed_min': None,
            'current_confirmed_max': None,
            'current_confirmed_min': None,
            'current_candidate_max': None,
            'current_candidate_min': None
        }
        ...
        # 1) Yeni candidate geldiyse işle
        if filtered_signal:
            stype = filtered_signal.get('type')
            idx = filtered_signal.get('idx')

            pos = self.safe_get_pos(df, idx)
            if pos is not None:

                # MAX CAND
                if stype == 'max':
                    high = float(df.iloc[pos]['high'])
                    if self.cand_max_idx[sym] is None:
                        self.cand_max_idx[sym] = idx
                        self.cand_max_price[sym] = high
                        self.cand_max_age[sym] = 0
                    else:
                        if high > self.cand_max_price[sym]:  # A seçeneği
                            self.cand_max_idx[sym] = idx
                            self.cand_max_price[sym] = high
                            self.cand_max_age[sym] = 0
                ...
                # MIN CAND
                    ...
                    if self.cand_min_idx[sym] is None:
                        self.candcand_min_idx[sym] = idx
                        self.cand_min_price[sym] = low
                        self.cand_min_age[sym] = 0
                    else:
                        if low < self.cand_min_price[sym]:
                            self.cand_min_idx[sym] = idx
                            self.cand_min_price[sym] = low
                            self.cand_min_age[sym] = 0

        # 2) Candidate yaşlandırma
        # MAX
        if self.cand_max_idx[sym] is not None:
            if last_high < self.cand_max_price[sym]:
                self.cand_max_age[sym] += 1
            else:
                self.cand_max_age[sym] = 0

            if self.cand_max_age[sym] >= self.confirm_bars:
                self.conf_max_idx[sym] = self.cand_max_idx[sym]
                self.conf_max_price[sym] = self.cand_max_price[sym]
                events['new_confirmed_max'] = (self.conf_max_idx[sym], self.conf_max_price[sym])
                self.cand_max_idx[sym] = None
                self.cand_max_price[sym] = 0.0
                self.cand_max_age[sym] = 0

        # MIN
        if self.cand_min_idx[sym] is not None:
            if last_low > self.cand_min_price[sym]:
                self.cand_min_age[sym] += 1
            else:
                self.cand_min_age[sym] = 0

            if self.cand_min_age[sym] >= self.confirm_bars:
                self.conf_min_idx[sym] = self.cand_min_idx[sym]
                self.conf_min_price[sym] = self.cand_min_price[sym]
                events['new_confirmed_min'] = (self.conf_min_idx[sym], self.conf_min_price[sym])
                self.cand_min_idx[sym] = None
                self.cand_min_price[sym] = 0.0
                self.cand_min_age[sym] = 0

        # 3) Confirmed break kontrolü
        if self.conf_max_idx[sym] is not None:
            if anlik_fiyat > self.conf_max_price[sym]:
                events['deleted_confirmed_max'] = (self.conf_max_idx[sym],)
                self.conf_max_idx[sym] = None
                self.conf_max_price[sym] = 0.0

        if self.conf_min_idx[sym] is not None:
            if anlik_fiyat < self.conf_min_price[sym]:
                events['deleted_confirmed_min'] = (self.conf_min_idx[sym],)
                self.conf_min_idx[sym] = None
                self.conf_min_price[sym] = 0.0

        # 4) Mevcut bilgileri döndür
        if self.conf_max_idx[sym] is not None:
            events['current_confirmed_max'] = (self.conf_max_idx[sym], self.conf_max_price[sym])
        if self.conf_min_idx[sym] is not None:
            events['current_confirmed_min'] = (self.conf_min_idx[sym], self.conf_min_price[sym])
        if self.cand_max_idx[sym] is not None:
            events['current_candidate_max'] = (
                self.cand_max_idx[sym], self.cand_max_price[sym], self.cand_max_age[sym]
            )
        if self.cand_min_idx[sym] is not None:
            events['current_candidate_min'] = (
                self.cand_min_idx[sym], self.cand_min_price[sym], self.cand_min_age[sym]
            )
...
pivot_engine = PivotEngine(confirm_bars=1)
...
while True:
    try:
        for sym in symbols:
            ...
            #  OHLC DATA AL
            # ----------------------------
            df = fetch_candlestick_data(sym, "3m", 350)
            ...
            # ===============================
            # PivotEngine’e sinyal gönder
            # ===============================
            filtered_signal = None
            if balanced_idx:
                last_idx = balanced_idx[-1]
                if last_idx in min_idx:
                    filtered_signal = {'type': 'min', 'idx': last_idx}
                elif last_idx in max_idx:
                    filtered_signal = {'type': 'max', 'idx': last_idx}

            pivot_events = pivot_engine.update(df, sym, filtered_signal)

            # Debug görmek istersen:
            if pivot_events['new_confirmed_max']:
                print(f"[PE] Yeni MAX Onay: {pivot_events['new_confirmed_max']}")
            if pivot_events['new_confirmed_min']:
                print(f"[PE] Yeni MIN Onay: {pivot_events['new_confirmed_min']}")
            ...
            # =====================================
            #   YENİ FIB PIVOT SEÇİMİ (Trend Tabanlı 1-2-3 Ayak)
            # =====================================

            pivots = build_pivots_from_filtered(df, balanced_idx, min_idx, max_idx)
            fib_extension_prices.setdefault(sym, {"Yukselen": [], "Dusen": []})

            fib_levels = []

            # Pivot sayısı çok azsa FIB yok
            if len(pivots) < 3:
                fib_extension_prices[sym][trend] = []
            else:
                # trend_start_idx pivotunun listedeki indexini bul
                idx_trend = None
                for i, p in enumerate(pivots):
                    if p["idx"] == trend_start_idx:
                        idx_trend = i
                        break

                # En az 2 pivot geriye gidilebilmeli
                if idx_trend is None or idx_trend < 2:
                    fib_extension_prices[sym][trend] = []
                else:
                    # 1., 2. ve 3. ayak pivotları
                    p1 = pivots[idx_trend - 2]  # Ayak 1
                    p2 = pivots[idx_trend - 1]  # Ayak 2
                    p3 = pivots[idx_trend]      # Ayak 3 = trend_start_idx

                    # ============================
                    #   YÜKSELEN TREND
                    # ============================
                    if trend == "Yukselen":
                        if p1["type"] == "min" and p2["type"] == "max" and p3["type"] == "min":
                    ...
                    # ============================
                        ...
                        if p1["type"] == "max" and p2["type"] == "min" and p3["type"] == "max":
                            if p3["price"] < p1["price"]:
                                start = p3["price"]
                                prev_min = p2["price"]
                                prev_max = p1["price"]
            ...
            # Debug
            if fib_levels:
                print(f"[FIB] {sym} → İlk 3 pivot: {p1['price']}, {p2['price']}, {p3['price']}")
                print(f"{sym} FIB:", [round(x, 6) for x in fib_levels[:10]])
            else:
                print(f"[FIB] {sym} → FIB oluşmadı.")
            ...
            if fib_levels:
                ...
                # ---------------------------
                #   DÜŞEN TREND ARALIK SEÇİMİ
                # ---------------------------
                if trend == "Dusen":
                    # fib_levels = yüksek → düşük sıralı
                    for i in range(len(fib_levels) - 1):
                        # fiyatın aralığa girmesi: ÜST >= fiyat > ALT
                        if fib_levels[i] >= fiyat > fib_levels[i + 1]:
                            ust_deger[sym] = fib_levels[i]
                            alt_deger[sym] = fib_levels[i + 1]
                            break

                # ---------------------------
                #   YÜKSELEN TREND ARALIK SEÇİMİ
                # ---------------------------
                else:  # Yukselen
                    # fib_levels = düşük → yüksek sıralı
                    for i in range(len(fib_levels) - 1):
                        # fiyatın aralığa girmesi: ALT <= fiyat < ÜST
                        if fib_levels[i] <= fiyat < fib_levels[i + 1]:
            ...
            # PivotEngine → Ana pivot değişkenlerine aktar
            if pivot_events['current_confirmed_max']:
                idx, price = pivot_events['current_confirmed_max']
                onceki_max_idx_price[sym] = son_max_idx_price.get(sym, 0.0)
                son_max_idx_price[sym] = price
                sabitlemis_max_sinyal[sym] = price

            if pivot_events['current_confirmed_min']:
                idx, price = pivot_events['current_confirmed_min']
                onceki_min_idx_price[sym] = son_min_idx_price.get(sym, 0.0)
                son_min_idx_price[sym] = price
                sabitlemis_min_sinyal[sym] = price
            ...
            # --------------------------------
                ...
                poz_kapandi = False  # Bu bar içinde pozisyon kapandı mı?

                # ==============================================
                # 1) BELİRSİZ TRENDE ÖZEL STOP KOŞULLARI
                # ==============================================

                # SHORT pozisyon: son MIN pivot + gc yukarı kırılırsa short kapat
                if data["shortPoz"] and son_min_idx_price.get(sym, 0) > 0:
                    if fiyat >= son_min_idx_price.get(sym, 0) + gc:
                        poz_miktar = get_position_qty(sym)
                        if poz_miktar > 0:
                            print(f"📤 {sym} BELİRSİZ TREND – SHORT kapatılıyor "
                                    f"(son MIN + gc kırıldı) @ {fiyat:.6f}")
                            shortExit(sym, poz_miktar)
                ...
                # LONG pozisyon: son MAX pivot - gc aşağı kırılırsa long kapat
                if data["longPoz"] and son_max_idx_price.get(sym, 0) > 0:
                    if fiyat <= son_max_idx_price.get(sym, 0) - gc:
                        poz_miktar = get_position_qty(sym)
                        if poz_miktar > 0:
                            print(f"📤 {sym} BELİRSİZ TREND – LONG kapatılıyor "
                                    f"(son MAX - gc kırıldı) @ {fiyat:.6f}")
                            longExit(sym, poz_miktar)
                ...
                # ==============================================
                # 2) EĞER BU BARDA POZ KAPANMADIYSA YENİ İŞLEM AÇ
                # ==============================================
